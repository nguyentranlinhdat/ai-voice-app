...
const VOICE_FILES: VoiceFile[] = [
  { name: "Michael", s3_key: "voices/michael_sample.wav" },
];
...
export default function CreatePage() {
  ...
  const [selectedVoice, setSelectedVoice] = useState(
    VOICE_FILES[0]?.s3_key ?? "samples/voice/Michael.wav",
  );
  ...
  useEffect(() => {
    const initializeData = async () => {
      try {
        //đồng thời lấy 3 dữ liệu riêng biệt session user, project âm thanh đã lưu trc đó và tệp voice âm thanh đã tải lên
        const [, projectsResult, voicesResult] = await Promise.all([
          authClient.getSession(),
          getUserAudioProjects(),
          getUserUploadedVoices(),
        ]);
        // check trạng thái âm thanh tải lên thàng công hay không
        ...
        <div className="border-b border-gray-200 bg-white py-2">
          <div className="mx-auto max-w-7xl text-center">
            ...
            <p className="text-muted-foreground mx-auto max-w-xl text-xs">
              Generator natural-sounding speech in multiple languages with voice
              cloning support.
            </p>
          </div>
        </div>
