...
import { Globe, Loader2, Settings, Upload, Volume2 } from "lucide-react";
import { Card, CardContent } from "~/components/ui/card";
import type { Language, UploadedVoice, VoiceFile } from "~/types/tts";
        ...
        <div className="mb-3 flex items-start justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-xs">
              Customize your speech generation settings
            </p>
          </div>
        </div>
        <div className="space-y-3">
          <div>
            <label className="mb-1 flex items-center gap-1 text-xs font-semibold">
              <Globe className="h-3 w-3"></Globe>Language
            </label>
            <select
              value={selectedLanguage}
              onChange={(e) => setSelectedLanguage(e.target.value)}
              className="boder-input bg-background w-full rounded-md border px-2 py-1.5 text-xs"
            >
              {languages.map((lang) => (
          ...
          <div>
            <label className="mb-1 flex items-center gap-1 text-xs font-semibold">
              <Volume2 className="h-3 w-3"></Volume2>Voice
            </label>
            <select
              value={selectedVoice}
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground text-xs">
                Upload a clear voice sample (WAV/MP3). Uploaded voice appear in
                the dropdown above.
              </p>
            </div>
          </div>
